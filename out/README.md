...
- `config/deploy.yml`
- `config/deploy.orders.yml`
- `config/deploy.web.yml`
- `.kamal/secrets` — secret lookups (no literal values)
- `Dockerfile.api` — generated multi-stage .NET build
- `Dockerfile.orders` — generated multi-stage .NET build
- `Dockerfile.web` — generated multi-stage .NET build
...
```sh
kamal deploy
kamal deploy -c config/deploy.orders.yml
kamal deploy -c config/deploy.web.yml
```

Run all commands from this directory; each app has its own config file.
## Accessories
